    ...
    <view class="publish-page">
        ...
        <view class="header-navbar">
            <view class="navbar-content">
                ...
                <view class="navbar-title">发布二手房</view>
                <view class="navbar-right"></view>
            </view>
        </view>

        <scroll-view class="content-scroll" scroll-y>
            <view class="tip-banner sell">
                <view class="tip-title">1000+房东选择，提升曝光快速卖房</view>
                <view class="tip-sub">
                    <text>真实房源保障</text>
                    <text>专属经纪人服务</text>
                    <text>多平台同步展示</text>
            ...
            <view class="form-section">
                ...
                <view class="form-item required">
                    ...
                    <view class="form-value price-row">
                        ...
                        />
                        <text class="suffix-text">万元</text>
                        <view class="price-negotiable" @click="form.priceNegotiable = !form.priceNegotiable">
                ...
                <view class="form-item">
                    <view class="form-label">建成年代</view>
                    <view class="form-value">
                        <input
                            class="form-input"
                            type="text"
                            v-model="form.yearBuilt"
                            placeholder="请填写建成年代，如2020"
                            placeholder-style="color:#CCCCCC;"
                        />
                    </view>
                </view>
                ...
                <view class="form-item">
                    <view class="form-label">朝向(多选)</view>
                    <view class="orientation-chips">
                        <view
                            v-for="(item, idx) in orientationOptions"
                            :key="idx"
                            class="chip"
                            :class="{ active: form.orientation.indexOf(item) >= 0 }"
                            @click="toggleOrientation(item)"
                        >
                            {{ item }}
                        </view>
                    </view>
                ...
                <view class="form-item">
                    <view class="form-label">是否满五/满二</view>
                    <view class="full-five-chips">
                        <view
                            class="chip"
                            :class="{ active: form.fullFive }"
                            @click="form.fullFive = !form.fullFive"
                        >
                            满五
                        </view>
                        <view
                            class="chip"
                            :class="{ active: form.fullTwo }"
                            @click="form.fullTwo = !form.fullTwo"
                        >
                            满二
                        </view>
                    </view>
                ...
                <view class="form-item textarea-item">
                    <view class="form-label">核心卖点</view>
                    <textarea
                        class="form-textarea"
                        v-model="form.description"
                        placeholder="请介绍房源亮点、户型结构、周边配套等"
                        maxlength="500"
                        placeholder-style="color:#CCCCCC;"
                    ></textarea>
...
<script>
import { submitPropertyListing } from '@/api/store'
import { uploadFile, uploadVideo } from '@/utils/tools'

export default {
    name: 'HousePublishSell',
    data() {
        return {
            statusBarHeight: 0,
    ...
    computed: {
        unitPrice() {
            if (this.form.priceNegotiable) return ''
            const area = parseFloat(this.form.area)
            const total = parseFloat(this.form.totalPrice)
            if (!area || !total) return ''
            const price = (total * 10000) / area
    ...
    methods: {
        ...
        chooseTencentLocation() {
            ...
            uni.chooseLocation({
                ...
                success: (res) => {
                    ...
                    if (res.address && !this.form.addressDetail) {
        ...
        toggleOrientation(item) {
            const i = this.form.orientation.indexOf(item)
            if (i >= 0) {
                this.form.orientation.splice(i, 1)
            } else {
                this.form.orientation.push(item)
            }
        },
        chooseVideo() {
            uni.chooseVideo({
        ...
        validate() {
            ...
            if (!this.form.priceNegotiable && !this.form.totalPrice) return '请填写总价或勾选是否面议'
        ...
        submit() {
            ...
            const doSubmit = (imageUrls) => {
                const payload = {
                    ...
                    year_built: this.form.yearBuilt,
...
.tip-banner {
    padding: 30rpx;
    background: linear-gradient(90deg, #FFE4D6 0%, #FFD3C0 100%);
}
